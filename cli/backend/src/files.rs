...
/// POST /api/files/copy {"from": "...", "to": "..."} — copy a file or folder
/// inside HOME (recursive for folders). Unlike rename, the source is kept.
/// Refuses to copy HOME itself or a folder into its own subtree.
#[derive(Deserialize)]
pub struct CopyBody {
    pub from: String,
    pub to: String,
...
fn copy_recursive(from: &std::path::Path, to: &std::path::Path) -> std::io::Result<()> {
    ...
    if meta.file_type().is_symlink() {
        // Re-link symlinks instead of following them (avoids cycles).
        #[cfg(unix)]
        {
            let target = std::fs::read_link(from)?;
            std::os::unix::fs::symlink(target, to)?;
        ...
        #[cfg(not(unix))]
        {
            // Fall back to copying the pointed-to file contents.
            std::fs::copy(from, to)?;
            return Ok(());
        }
    }
...
pub async fn api_copy_file(Json(b): Json<CopyBody>) -> Response {
    ...
    // Refuse to copy a folder into its own subtree (infinite recursion).
...
/// POST /api/files/chmod {"path", "mode"} — set unix permission bits.
/// `mode` is an octal string (`"755"`, `"0644"`) or a JSON number whose
/// decimal digits are read as octal (`644` → `0o644`).
#[derive(Deserialize)]
pub struct ChmodBody {
    pub path: String,
...
/// Parse user-supplied chmod mode into permission bits (0..=0o7777).
fn parse_chmod_mode(v: &serde_json::Value) -> Result<u32, String> {
    let digits = match v {
        serde_json::Value::String(s) => s.trim().to_string(),
        serde_json::Value::Number(n) => n.to_string(),
        _ => return Err("mode must be an octal string like \"755\"".to_string()),
    };
    let digits = digits
        .strip_prefix("0o")
        .or_else(|| digits.strip_prefix("0O"))
        .unwrap_or(&digits);
    // Allow the conventional single leading zero (`0755`); after that the
    // value must be 1..=4 octal digits.
    let digits = match digits.len() {
        4 if digits.starts_with('0') => &digits[1..],
        1 | 2 | 3 | 4 => digits,
        _ => return Err(format!("bad mode (want octal 0..7777): {digits}")),
    };
    if digits.is_empty() || digits.len() > 4 || !digits.bytes().all(|b| b.is_ascii_digit()) {
        return Err(format!("bad mode (want octal 0..7777): {digits}"));
    }
    let mode = u32::from_str_radix(digits, 8)
        .map_err(|_| format!("bad mode (want octal 0..7777): {digits}"))?;
    if mode > 0o7777 {
        return Err(format!("bad mode (want octal 0..7777): {digits}"));
    }
    Ok(mode)
...
pub async fn api_chmod_file(Json(b): Json<ChmodBody>) -> Response {
    let (_, target) = match resolve_inside_home(Some(&b.path)) {
        Ok(v) => v,
        Err((code, msg)) => return (code, msg).into_response(),
    };
    let mode = match parse_chmod_mode(&b.mode) {
        Ok(m) => m,
    ...
    #[cfg(unix)]
    {
        ...
        match std::fs::set_permissions(&target, perm) {
            Ok(()) => (
                StatusCode::OK,
                Json(serde_json::json!({ "ok": true, "path": target.to_string_lossy(), "mode": format!("{mode:03o}") })),
            )
                .into_response(),
            Err(e) => (
                StatusCode::INTERNAL_SERVER_ERROR,
